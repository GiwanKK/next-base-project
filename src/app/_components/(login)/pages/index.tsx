...
import Login from "@/app/(beforeLogin)/page";
...
const LoginPage = ({ hasAuth }: { hasAuth: boolean }) => {
  ...
  const passwordRegister = register("password", {
    ...
    minLength: {
      value: 9,
      message: "대소문자, 숫자, 특수문자를 포함한 9자~16자를 입력해 주세요.",
    },
    maxLength: {
      value: 16,
      message: "대소문자, 숫자, 특수문자를 포함한 9자~16자를 입력해 주세요.",
  ...
  const onSubmit: SubmitHandler<Login> = async (data) => {
    ...
    try {
      const response = await axios({
        url: `${process.env.NEXT_PUBLIC_TQM_ADMIN_SERVER_URL}/api/v1/login`,
        method: "POST",
        headers: {
